  ...
  case PTP_LISTENING:
   ...
   case PTP_MASTER:
    ...
    timerStart(&ptpPortDS->timers.sync, 
	       8000 * pow_2(ptpPortDS->logSyncInterval));
    timerStart(&ptpPortDS->timers.announceInterval, 
	       3000 * pow_2(ptpPortDS->logAnnounceInterval));
    timerStart(&ptpPortDS->timers.pdelayReq, 
	       2000 * pow_2(ptpPortDS->logMinPdelayReqInterval));
...
//			clearForeignMasters(ptpPortDS);	TODO(1): check why it's not working with this, investigate the standard
		...
		else if(rtOpts->P2P_mode)
		{
		    if(timerExpired(&ptpPortDS->timers.pdelayReq))
			{
				PTPD_TRACE(TRACE_PROTO, ptpPortDS,"PDELAYREQ_INTERVAL_TIMEOUT_EXPIRED\n");
                    ...
                    //else
				    //    PTPD_TRACE(TRACE_PROTO, ptpPortDS,"Waiting for FOLLOW UP\n");
			}
		}
		break;
	...
	case PTP_MASTER:
		...
		{
            PTPD_TRACE(TRACE_PROTO, ptpPortDS,"!!!!!!!!SYNC TIME OUT \n");
        ...
        /*if (rtOpts->P2P_mode)
		{
			if(timerExpired(&ptpPortDS->timers.pdelayReq))
			{
				...
				//essuePDelayReq(rtOpts,ptpPortDS);
                 issuePDelayResp(&time,&ptpPortDS->PdelayRespHeader,rtOpts,ptpPortDS);
                if(ptpPortDS->pDelayResp_tx_ts.correct)
                    issuePDelayRespFollowUp(&time,&ptpPortDS->PdelayRespFollowupHeader,rtOpts,ptpPortDS);
			}
		}*/
...
// new 
        /*if(rtOpts->P2P_mode)
        {
            if(timerExpired(&ptpPortDS->timers.pdelayResp))
            {
                  
				PTPD_TRACE(TRACE_PROTO, ptpPortDS,"event PDELAYRESP_INTERVAL_TIMEOUT_EXPIRES\n");
                issuePDelayResp(&time,&ptpPortDS->PdelayRespHeader,rtOpts,ptpPortDS);
                if(ptpPortDS->pDelayResp_tx_ts.correct)
                    issuePDelayRespFollowUp(&time,&ptpPortDS->PdelayRespHeader,rtOpts,ptpPortDS);
...
void handle(RunTimeOpts *rtOpts, PtpPortDS *ptpPortDS)
{
	...
	*/
	...
	case SYNC:
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle ..... SYNC received in handle function\n");
		handleSync(&ptpPortDS->msgTmpHeader, ptpPortDS->msgIbuf, length, &time, isFromSelf, rtOpts, ptpPortDS);
        TRACE_DEV("###T2### SLAVE Rx Symc-> utc %d nsc %d phase %d\n",  
                 (int32_t)ptpPortDS->current_rx_ts.utc, ptpPortDS->current_rx_ts.nsec, ptpPortDS->current_rx_ts.phase);
		break;

	case FOLLOW_UP:
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle ..... FOLLOW_UP received in handle function\n");
		handleFollowUp(&ptpPortDS->msgTmpHeader, ptpPortDS->msgIbuf, length, isFromSelf, rtOpts, ptpPortDS);
        		break;

	case DELAY_REQ:
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle ..... DELAY_REQ received in handle function\n");
		handleDelayReq(&ptpPortDS->msgTmpHeader, ptpPortDS->msgIbuf, length, isFromSelf, rtOpts, ptpPortDS);
		break;

#ifndef WR_MODE_ONLY
	case PDELAY_REQ:
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle ..... PDELAY_REQ received in handle function\n");
	...
	case PDELAY_RESP:
	    ...
	    TRACE_DEV("###T6### SLAVE Rx PDELAY_RESP-> utc %d nsc %d phase %d\n",  
                 (int32_t)ptpPortDS->current_rx_ts.utc, ptpPortDS->current_rx_ts.nsec, ptpPortDS->current_rx_ts.phase);
		break;
	...
	case DELAY_RESP:
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle ..... DELAY_RESP\n");
			...
			if (msgIsFromCurrentParent(header, ptpPortDS))
			{
				...
				*/
				...
				if ((header->flagField[0] & 0x02) == TWO_STEP_FLAG)
				{
					...
					PTPD_TRACE(TRACE_PROTO, ptpPortDS,"Correction Field in SYNC....: Seconds:  %d Nanosec: %d \n", \
		...
		if (msgIsFromCurrentParent(header, ptpPortDS))
		{
			if (ptpPortDS->waitingForFollow)
			{
				if ((ptpPortDS->recvSyncSequenceId == header->sequenceId))
				{
					...
					/*PTPD_TRACE(TRACE_PROTO, ptpPortDS," FOLLOW_UP:  Correction Field: Sec  %d nano %d phase %d \n", correctionField.seconds, \
                                                                                                                    correctionField.nanoseconds, \ 
                                                                                                                    correctionField.phase);*/
					wr_servo_got_sync(ptpPortDS, preciseOriginTimestamp, ptpPortDS->sync_receive_time);
                    ptpPortDS->sync_receive_time.correct=1;
                    TRACE_DEV("###T1### SLAVE Rx FOLLOW_UP -> utc %d nsc %d phase %d\n",
                             (int32_t)preciseOriginTimestamp.seconds, preciseOriginTimestamp.nanoseconds, preciseOriginTimestamp.phase);
   ...
   if(rtOpts->P2P_mode)
   {
	...
	switch(ptpPortDS->portState )
	{
		...
		case PTP_MASTER:
           ...
           TRACE_DEV("###T4### MASTER Rx PDELAY_REQ-> utc %d nsc %d phase %d\n",  
                 (int32_t)ptpPortDS->current_rx_ts.utc, ptpPortDS->current_rx_ts.nsec, ptpPortDS->current_rx_ts.phase);
           ...
           //PTPD_TRACE(TRACE_PROTO, ptpPortDS,"sending ..... PDELAY_RESP:, succedded\n");
           //
           issuePDelayResp(time,&ptpPortDS->PdelayRespHeader,rtOpts,ptpPortDS);
           
           //PTPD_TRACE(TRACE_PROTO, ptpPortDS,"sending ..... PDELAY_FOLLOW_UP:, succedded\n");
           issuePDelayRespFollowUp(time,&ptpPortDS->PdelayFollowupHeader,rtOpts,ptpPortDS);
	...
	switch(ptpPortDS->portState )
	{
		...
		case PTP_SLAVE:
		    ...
		    if (isFromSelf)
		    {
                ...
                //PTPD_TRACE(TRACE_PROTO, ptpPortDS,"*** PdelayReqHeader from SLAVE FROM SELF" );
            ...
            * WR: HW timestamps ##T4##
            */
            msgUnpackPDelayResp(ptpPortDS->msgIbuf,&ptpPortDS->msgTmp.preq);
            toInternalTime(&requestReceiptTimestamp,&ptpPortDS->msgTmp.preq.originTimestamp);
            ptpPortDS->pdelay_req_receive_time = requestReceiptTimestamp;

            /*
            * WR: HW timestamps ##T6##
            */
            ptpPortDS->pdelay_resp_receive_time.seconds = ptpPortDS->current_rx_ts.utc;
			...
			//PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle PDelayResp msg, succedded" );
...
if (!rtOpts->E2E_mode)
 {
	...
	switch(ptpPortDS->portState )
	{
		...
		if (header->sequenceId == ptpPortDS->sentPDelayReqSequenceId-1)
		{
            ...
            wr_servo_got_delay_P2P(ptpPortDS, header->correctionfield.lsb);
            wr_servo_update(ptpPortDS);
...
void issueFollowup(RunTimeOpts *rtOpts,PtpPortDS *ptpPortDS)
{
	...
	{
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"issue  ..... FOLLOW_UP: succedded [sending time of sync tx: sec = %d nanosec = %d]\n",\
...
void issuePDelayReq(RunTimeOpts *rtOpts,PtpPortDS *ptpPortDS)
{
	...
	if (!netSendEvent(ptpPortDS->msgObuf,PDELAY_REQ_LENGTH,&ptpPortDS->netPath,&ptpPortDS->pDelayReq_tx_ts))
	{
		toState(PTP_FAULTY,rtOpts,ptpPortDS);
	...
	{
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"issue  ..... PDELAY_REQ, succedded [TS %s] \n",\
                                           format_wr_timestamp(ptpPortDS->pDelayReq_tx_ts));
...
void issuePDelayResp(TimeInternal *time,MsgHeader *header,RunTimeOpts *rtOpts,PtpPortDS *ptpPortDS)
{
	...
	if (!netSendEvent(ptpPortDS->msgObuf,PDELAY_RESP_LENGTH,&ptpPortDS->netPath,&ptpPortDS->pDelayResp_tx_ts))
	{
		toState(PTP_FAULTY,rtOpts,ptpPortDS);
	...
	{
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"issue  ..... PDELAY_RESP, succedded Correct %d \n",\
                                           ptpPortDS->pDelayResp_tx_ts.correct);         

        ptpPortDS->sentPDelayRespSequenceId++;
	}
...
void issuePDelayRespFollowUp(TimeInternal *time,MsgHeader *header,RunTimeOpts *rtOpts,PtpPortDS *ptpPortDS)
{
	...
	if (!netSendGeneral(ptpPortDS->msgObuf,PDELAY_RESP_FOLLOW_UP_LENGTH,&ptpPortDS->netPath))
	{
		toState(PTP_FAULTY,rtOpts,ptpPortDS);
	...
	{
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"issue  ..... PDELAY_RESP_FOLLOW_UP, succedded\n");
        
        //sending time of pDelayResp tx]: \n\t\t sec = %lld \n\t\t  nanosec = %lld\n",\
		//(unsigned long long)ptpPortDS->pDelayResp_tx_ts.utc,\
		//(unsigned long long)ptpPortDS->pDelayResp_tx_ts.nsec);
	}

		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"---------------------Finished--------------\n");
}
#endif
