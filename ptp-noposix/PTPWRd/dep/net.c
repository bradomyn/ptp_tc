...
const mac_addr_t PTP_P2P_ADDR       = {0x01, 0x80, 0xc2, 0 , 0, 0x0e};
const mac_addr_t ZERO_ADDR          = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
Boolean netInit(NetPath *netPath, RunTimeOpts *rtOpts, PtpPortDS *ptpPortDS)
{
  ...
  // TEST
  memcpy(bindaddr.mac, PTP_P2P_ADDR, sizeof(mac_addr_t));
 
  netPath->wrSockP2P = ptpd_netif_create_socket(PTPD_SOCK_RAW_ETHERNET, 0, &bindaddr);

  if(netPath->wrSockP2P ==  NULL)
  {
    return FALSE;
  }
  ...
  //For P2P Transparent Clock
  //memcpy(netPath->peerMulticastAddr.mac, PTP_MULTICAST_ADDR,  sizeof(mac_addr_t));
  memcpy(netPath->peerMulticastAddr.mac, PTP_P2P_ADDR,  sizeof(mac_addr_t));
...
//TEST
    if(!(ret>0))
        ret = ptpd_netif_recvfrom(netPath->wrSockP2P, &from_addr, buf, 1518, current_rx_ts);
//TEST
    if(ret < 0)
        return ret;
  ...
  //TRACE_DEV("************netRecvMsg******** utc %d nsc %d phase %d\n",
  //  (int32_t)current_rx_ts->utc, current_rx_ts->nsec, current_rx_ts->phase);
...
ssize_t netSendEvent(Octet *buf, UInteger16 length, NetPath *netPath, wr_timestamp_t *current_tx_ts)
{
  ...
  TRACE_DEV("************netSendEvent******** utc %d nsc %d phase %d correct %d\n",
   (int32_t)current_tx_ts->utc, current_tx_ts->nsec, current_tx_ts->phase, current_tx_ts->correct);
  return (ssize_t)ret;

}
/*Sending Sync and FollowUp with P2P address */
ssize_t netSendPeerEvent(Octet *buf,UInteger16 length,NetPath *netPath,wr_timestamp_t *current_tx_ts)
{
  ...
  if(ret <= 0)
      PTPD_TRACE(TRACE_ERROR, NULL,"error sending P2P multi-cast event message\n");
