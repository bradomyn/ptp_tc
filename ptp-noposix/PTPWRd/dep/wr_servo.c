...
int wr_servo_got_delay_P2P(PtpPortDS *clock, Integer32 cf)
{
	wr_servo_state_t *s = &clock->wr_servo;
	...
	s->t5.correct = 1; // clock->pdelay_resp_send_time.correct;

    s->t6 = timeint_to_wr(clock->pdelay_resp_receive_time);
    s->t6.correct = clock->pdelay_resp_receive_time.correct;

    clock->pDelayReq_tx_ts.correct = 0;
    clock->pdelay_resp_receive_time.correct = 0;
...
int wr_servo_update(PtpPortDS *clock)
{
    ...
    PTPD_TRACE(TRACE_PROTO, NULL, "&&&&&&&&& Servo Update &&&&&&&\n");

    PTPD_TRACE(TRACE_PROTO, NULL, "servo:t1 %d t2 %d t3 %d t4 %d t5 %d t6 %d\n", \
    ...
    if(!s->t1.correct || !s->t2.correct) 
    {
        ...
        if(!s->t5.correct || !s->t6.correct || !s->t3.correct || !s->t4.correct)
        {

            PTPD_TRACE(TRACE_PROTO, NULL, "&&&&&&&&& T1 T2 / T3 T4 T5 Error\n");
            return 0;

        }
        else // The sync is done between MASTER -> TC or TC -> TC, TC -> SLAVE, Point to Point Peer Delay
        {
            PTPD_TRACE(TRACE_PROTO, NULL, "&&&&&& Point ot Point Peer Delay\n");
            ...
            //ts_offset = ts_add(ts_sub(s->t5, s->t6), picos_to_ts(delay_ms_fix));
            ts_offset = ts_add(ts_sub(s->t1, s->t2), picos_to_ts(delay_ms_fix));
            ts_offset_hw = ts_hardwarize(ts_offset, s->clock_period_ps);
        }
    }
    else if(!s->t5.correct || !s->t6.correct) 
    {

        if(!s->t3.correct || !s->t4.correct)
        {
            PTPD_TRACE(TRACE_PROTO, NULL, "&&&&&&&&& T3 T4 T5 T6 ERROR \n");
            return 0;

        }
        else // DDR mechanism
        {
            PTPD_TRACE(TRACE_SERVO, NULL, "servo: DDR\n"); 
    ...
    else  if(!s->t3.correct || !s->t4.correct)
    {
        PTPD_TRACE(TRACE_PROTO, NULL, "&&&&&&&&& T3 T4 NOOOO 3\n");

       return 0;
    }
    else // The sync is done between MASTER -> SLAVE, End to End Peer Delay
    {
        PTPD_TRACE(TRACE_PROTO, NULL, "&&&&&& End to Edd Peer Delay\n");
    ...
    if (1) { /* enable for debugging */
        ...
        dump_timestamp("->t5", s->t5);
        dump_timestamp("->t6", s->t6);
